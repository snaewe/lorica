...
REQUIRED_LIBTOOL_VERSION=${REQUIRED_LIBTOOL_VERSION:-1.5.0}
...
configure_files="`find $srcdir -name '{arch}' -prune -o -name ACE_wrappers -prune -o -name '_darcs' -prune -o -name '.??*' -prune -o -name configure.ac -print -o -name configure.in -print`"
...
for configure_ac in $configure_files; do 
	...
	if [ $REQUIRED_AUTOMAKE_VERSION != 1.4 ]; then
	    $AUTOMAKE --warnings=all --Werror --gnu --add-missing --force --copy || exit 1
	else
	    $AUTOMAKE --warnings=all --Werror --gnu --add-missing --copy || exit 1
	fi
