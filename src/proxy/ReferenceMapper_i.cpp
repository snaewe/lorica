	...
	try
	{
		if (this->has_security_)
		{
			CORBA::Object_var obj =
				orb_->resolve_initial_references("SecurityLevel2:SecurityManager");
			...
			this->access_decision_ =
				TAO::SL2::AccessDecision::_narrow(sl2ad.in());
		}
		else
	...
	throw (CORBA::SystemException)
{
	Lorica::ReferenceMapValue_var rmv =
		this->registry_->map_reference (orig, true, require_secure);
	...
	{
		CORBA::String_var mapped =
			this->orb_->object_to_string(rmv->mapped_ref_.in());
		if(Lorica_debug_level > 5)
			ACE_DEBUG ((LM_DEBUG,
				    "(%P|%t) Lorica::ReferenceMapper_i::as_server_i "
...
{
	Lorica::ReferenceMapValue_var rmv =
		this->registry_->remove_reference (mapped, false);
}
